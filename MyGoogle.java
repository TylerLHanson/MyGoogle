...
public class MyGooglePA07 {
   ...
   **/
   public static String[] findMatches(String searchPhrase, String[] lines){
      ...
      //initialized an integer at zero 
      int numberOfMatches = 0;

      //the number of elements in the new array will be equal to the number of matches
      String[] matches = new String[numberOfMatches];

      //loop through every element of the parameter array
      //if the match is successful, insert the element in the matches array
      for (int i=0; i<lines.length; i++){
        if (lines[i].indexOf(searchPhrase)>-1)
          numberOfMatches++;
          matches[numberOfMatches] = lines[i];
      }
      return matches;
   }
   ...
   **/
   public static String[] findLinks(String[] lines){
      ...
      //the number of elements in the new array will be equal to the number of URLnames found
      String[] URL = new String[numberOfLinks];

      //loop through every element of the parameter array
      //if a URL if found, insert the element in the URL array
      for (int i=0; i<lines.length; i++){
         int p = lines[i].indexOf("<a href=\"http:");
            if (p > -1)
            numberOfLinks++;
            URL[numberOfLinks] = lines[i];
      }
      return URL;
   }
   ...
   **/
   public static String getLink(String line){
      ...
      //find the start and end of the actual URL
      int p = line.indexOf("<a href=\"http:");
      int start = p+9;
      int end = line.indexOf("\"",p+9);

      //create a new substring for the actual URL and return it
   ...
   **/
   public static void printMatches(String searchPhrase, String[] matches){
      ...
      //loop through every element of the matches array
      //print each element out telling the user it's a match 
      //formatted to disaply 50 characters of context 
      for (int i = 0; i<matches.length; i++){
        System.out.printf("**** MATCH: %50s", matches[i]);
      }
   }
   ...
   **/
   public static String getRandomLink(String[] links){
      ...
      //determine how many links are available
      int arrayLength = links.length;

      //pick a random integer between zero and the number of links available
      Random r = new Random();
      int randomNumber = r.nextInt(arrayLength);

      //set a new string equal to the randomNumber-th element in the array
      String randomLink = links[randomNumber];
